...
// (Keanu)(Person name:Keanu Reeves)から(TheMatrix)(Movie title:The Matrix)へリレーション作成
// リレーション名ACTED_INで、roles属性にNeoという値を設定
...
// Personラベルのノードname:"Emil Eifrem"を作成
// EmilからMovieラベルのノードtitle:'The Matrix'にACTED_INのリレーションを設定
MERGE (Emil:Person {name:"Emil Eifrem", born:1978})
MERGE (Emil)-[:ACTED_IN {roles:["Emil"]}]->(TheMatrix)

// 上記のThe Matrix同様、MovieラベルのノードThe Matrix Reloadedを作成
// その後、各PersonラベルのノードからMovieラベルのノードへリレーションを設定
// EmilからThe Matrix Reloadedへのリレーションは設定していない
...
// 新しくMovieラベルにtitle:"The Devil's Advocate"を追加
// 併せて、Personラベルに3つのノードを追加
// 上記で作成したPersonラベルのノード(Keanu)と下記で新たに追加した3つのPersonラベルのノードから
// Movieラベルtitle:"The Devil's Advocate"にリレーションを追加
...
// 上記同様、Movieラベルに1つのノード、Personラベルに13のノードを追加
// 13のPersonラベルのノードから1つのMovieラベルのノードにリレーションを追加
// Personラベルのノード(AaronS)は、2つのリレーションACTED_INとWROTEを持つ
...
// Personラベル 8ノード 追加
// 12リレーション 追加
// (※内2つは上記で追加した(TomC),(CubaG)、また(CameronC)は同一Movieラベルのノードに3つのリレーションを設定)
...
// Movieラベル 1ノード 追加
// Personラベル 5ノード 追加
// 8リレーション 追加(※内3つは上記で追加した(KieferS),(JerryO),(RobR))
...
// Movieラベル 1ノード 追加
// Personラベル 3ノード 追加
// 5リレーション 追加(※内2つは上記で追加した(JackN),(CubaG))
